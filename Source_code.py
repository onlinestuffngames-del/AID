...
REQUIRED_HEADERS   = ["denomination", "diameter_mm", "thickness_mm", "link"]

class VoiceAssistant:
    def speak(self, text: str, volume: float = 0.5) -> None:
        if not TTS_AVAILABLE or volume <= 0:
            return
        threading.Thread(target=self._run, args=(text, volume), daemon=True).start()

    @staticmethod
    def _run(text: str, volume: float) -> None:
        try:
            engine = pyttsx3.init()
...
class ArduinoController:
    ...
    def connect(self) -> tuple[bool, str]:
        ...
        try:
            self._serial = serial.Serial(
                self.selected_port, self.BAUD_RATE, timeout=self.TIMEOUT
            )
            time.sleep(2)
            self.start_reader()
            return True, f"Connected to {self.selected_port}."
    ...
    def send(self, command_name: str) -> tuple[bool, str]:
        if self._serial is None or not self._serial.is_open:
            return False, "Arduino not connected."
        try:
            self._serial.write((command_name + "\n").encode("utf-8"))
            return True, f"Sent: {command_name}"
        except Exception as exc:
...
class CoinDatabase:
    ...
    def __init__(self, path: Path) -> None:
        self.path    = path
        self.headers: list[str]            = list(REQUIRED_HEADERS)
        self.rows:    list[dict[str, str]] = []
        self._load()
        
    def _load(self) -> None:
        if not self.path.exists():
            self.headers = list(REQUIRED_HEADERS)
            self.rows    = []
            return
        with self.path.open(newline="", encoding="utf-8") as f:
            reader       = csv.DictReader(f)
            self.headers = list(reader.fieldnames or REQUIRED_HEADERS)
    ...
    @staticmethod
    def confidence_diameter(
        d1: float, d2: float | None, tolerance: float = 1.2
    ) -> float:
        if d2 is None:
            return 0.0
        diff = abs(d1 - d2)
    ...
    def best_matches(
        self, diameter_mm: float, limit: int = 3
    ) -> list[tuple[dict[str, str], float]]:
        scored: list[tuple[dict[str, str], float]] = []
        for row in self.rows:
            db_diam = self.parse_float(row.get("diameter_mm", ""))
            conf    = self.confidence_diameter(diameter_mm, db_diam)
            scored.append((row, conf))
        scored.sort(key=lambda x: x[1], reverse=True)
        return scored[:limit]

    def append_capture(
        self, image_path: Path, diameter_mm: float, location: str = ""
    ) -> None:
        ts      = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
        new_row = {h: "" for h in self.headers}
        new_row["link"]        = str(image_path)
        new_row["diameter_mm"] = f"{diameter_mm:.2f}"
...
class CameraMeasurement:
    ...
    BLUR_KERNEL          = 7
    HOUGH_DP             = 1.2
    HOUGH_MIN_DIST_RATIO = 0.25
    HOUGH_PARAM1         = 100
    HOUGH_PARAM2         = 30
    MIN_RADIUS_RATIO     = 0.05
    MAX_RADIUS_RATIO     = 0.45

    def __init__(self, reference_diameter_mm: float = 23.75) -> None:
    ...
    def start(self, index: int = 0) -> tuple[bool, str]:
        cap = cv2.VideoCapture(index)
        if not cap.isOpened():
            return False, f"Cannot open camera {index}."
        self._cap = cap
    ...
    def detect_main_circle(
        ...
        h, w  = frame.shape[:2]
        min_r = int(min(h, w) * self.MIN_RADIUS_RATIO)
        max_r = int(min(h, w) * self.MAX_RADIUS_RATIO)
        circles = cv2.HoughCircles(
            blurred,
            cv2.HOUGH_GRADIENT,
            dp        = self.HOUGH_DP,
            minDist   = int(min(h, w) * self.HOUGH_MIN_DIST_RATIO),
    ...
    def calibrate_from_frame(self) -> tuple[bool, str]:
        with self._lock:
        ...
        if circle is None:
            return False, "No circle detected in the current frame."
        _, _, r            = circle
        diameter_px        = r * 2.0
        self.reference_diameter_px = diameter_px
        self.mm_per_pixel          = self.reference_diameter_mm / diameter_px
        return (
            True,
            f"Calibrated: {self.mm_per_pixel:.4f} mm/pixel "
            f"(ref {diameter_px:.1f} px)",
        )

    def measure_diameter(
        self, sample_count: int = MEASUREMENT_FRAMES
        ...
        for _ in range(sample_count):
            ...
            if self.mm_per_pixel is not None:
                d_mm = diameter_px * self.mm_per_pixel
                cv2.putText(
                    annotated, f"{d_mm:.2f} mm",
                    (cx - 40, cy - r - 10),
        ...
        self.last_diameter_px = median_px

        if self.mm_per_pixel is None:
            return None, last_annotated

        return median_px * self.mm_per_pixel, last_annotated
    
    @staticmethod
    def save_capture(folder: Path, frame: np.ndarray) -> Path:
        folder.mkdir(parents=True, exist_ok=True)
        ts   = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        path = folder / f"coin_{ts}.jpg"
...
def compute_coin_volume_mm3(
    diameter_mm: float, thickness_mm: float | None
) -> float | None:
    if thickness_mm is None or diameter_mm <= 0 or thickness_mm <= 0:
        return None
    return math.pi * (diameter_mm / 2.0) ** 2 * thickness_mm
...
class DeviceInterface:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.arduino = ArduinoController(
            line_callback=lambda line: self.root.after(
                0, lambda v=line: self.handle_arduino_line(v)
            )
        )
    ...
    def load_database(self) -> tuple[list[str], list[dict[str, str]]]:
        db = CoinDatabase(CSV_PATH)
        return db.headers, db.rows
    ...
    def create_interface(self) -> None:
        ...
        self.left_col = tk.Frame(self.main, bg=bg)
        self.left_col.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 30))

        self.label_init = tk.Label(
            self.left_col, text="INIT",
            font=("Arial", 14, "bold"), bg=bg, fg=fg,
        )
        self.label_init.pack(anchor="w", pady=2)

        self.port_btn = tk.Button(
            self.left_col, text="3", font=("Arial", 12),
            bg="#d9d9d9", fg=fg, width=8,
            relief=tk.SUNKEN, bd=2, command=self.show_ports,
        )
        self.port_btn.pack(anchor="w", pady=5)

        self.place_btn = tk.Button(
            self.left_col, text="Scan Artifact", font=("Arial", 12),
            bg=btn, fg=fg, width=18, height=2,
            relief=tk.RAISED, bd=2, command=self.capture_and_catalog,
        )
        self.place_btn.pack(anchor="w", pady=10)

        self.scan_btn = tk.Button(
            self.left_col, text="Place Artifact", font=("Arial", 12),
            bg=btn, fg=fg, width=18, height=2,
            relief=tk.RAISED, bd=2, command=self.toggle_camera,
        )
        self.scan_btn.pack(anchor="w", pady=5)

        self.calibrate_btn = tk.Button(
            self.left_col, text="Calibrate (C)", font=("Arial", 11, "bold"),
            bg=btn, fg=fg, width=18,
            relief=tk.RAISED, bd=2, command=self.calibrate_from_current,
        )
        self.calibrate_btn.pack(anchor="w", pady=5)

        self.db_btn = tk.Button(
            self.left_col, text="Database", font=("Arial", 11, "bold"),
            bg=btn, fg=fg, width=18,
            relief=tk.RAISED, bd=2, command=self.open_database_window,
        )
        self.db_btn.pack(anchor="w", pady=5)

        self.site_btn = tk.Button(
            self.left_col, text="Catalog site", font=("Arial", 11, "bold"),
            bg=btn, fg=fg, width=18,
            relief=tk.RAISED, bd=2, command=self.open_catalog_site,
        )
        self.site_btn.pack(anchor="w", pady=5)

        self.location_label = tk.Label(
            self.left_col, text="Discovery location",
            font=("Arial", 11), bg=bg, fg=fg,
        )
        self.location_label.pack(anchor="w", pady=(10, 4))
        self.location_var = tk.StringVar(value="")
        self.location_entry = tk.Entry(
            self.left_col, textvariable=self.location_var,
            font=("Arial", 11), width=30,
        )
        self.location_entry.pack(anchor="w", pady=(0, 6))

        self.label_vol = tk.Label(
            self.left_col, text="Voice assistant volume",
            font=("Arial", 11), bg=bg, fg=fg,
        )
        self.label_vol.pack(anchor="w", pady=(15, 5))

        self.vol_frame = tk.Frame(self.left_col, bg=bg)
        ...
        self.vol_slider = tk.Scale(
            ...
            variable=self.volume, bg=bg, fg=fg, length=250,
            showvalue=False, troughcolor=self.light_slider_bg,
            highlightbackground=bg,
        )
        self.vol_slider.pack(side=tk.LEFT)
        self.vol_value = tk.Label(
            self.vol_frame, textvariable=self.volume,
            width=3, bg=bg, fg=fg,
        )
        self.vol_value.pack(side=tk.LEFT, padx=5)

        self.auto_btn = tk.Button(
            self.left_col, text="AUTO", font=("Arial", 11, "bold"),
            bg=btn, fg=fg, width=10,
            relief=tk.RAISED, bd=2, command=self.set_auto_mode,
        )
        self.auto_btn.pack(anchor="w", pady=5)

        self.manual_btn = tk.Button(
            self.left_col, text="MANUAL", font=("Arial", 11, "bold"),
            bg=btn, fg=fg, width=10,
            relief=tk.RAISED, bd=2, command=self.set_manual_mode,
        )
        self.manual_btn.pack(anchor="w", pady=2)

        self.led_btn = tk.Button(
            self.left_col, text="LED OFF", font=("Arial", 11, "bold"),
            bg="#808080", fg="white", width=10,
            relief=tk.RAISED, bd=2, command=self.toggle_led,
        )
        self.led_btn.pack(anchor="w", pady=2)

        self.stop_btn = tk.Button(
            self.left_col, text="STOP", font=("Arial", 14, "bold"),
            bg="#ff4444", fg="white", width=12, height=1,
            relief=tk.RAISED, bd=3, command=self.emergency_stop,
        )
        self.stop_btn.pack(anchor="w", pady=15)

        self.theme_frame = tk.Frame(self.left_col, bg=bg)
        self.theme_frame.pack(anchor="w", pady=5)
        self.light_btn_theme = tk.Button(
            self.theme_frame, text="Light mode", font=("Arial", 10),
            bg=btn, fg=fg, width=10,
            relief=tk.RAISED, bd=2, command=self.light_mode,
        )
        self.light_btn_theme.pack(side=tk.LEFT, padx=2)
        self.dark_btn_theme = tk.Button(
            self.theme_frame, text="Dark mode", font=("Arial", 10),
            bg=btn, fg=fg, width=10,
            relief=tk.RAISED, bd=2, command=self.dark_mode,
        )
        self.dark_btn_theme.pack(side=tk.LEFT, padx=2)

        self.right_col = tk.Frame(self.main, bg=bg, width=340)
        self.right_col.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)
        self.right_col.pack_propagate(False)

        self.cam_frame = tk.Frame(
            self.right_col, bg="black", relief=tk.RAISED, bd=3
        )
        self.cam_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 20))
        self.cam_label = tk.Label(
            self.cam_frame, text="CAM FEED",
            font=("Arial", 16, "bold"), fg="white", bg="black",
        )
        self.cam_label.pack(expand=True, fill=tk.BOTH)

        self.date_frame = tk.Frame(self.right_col, bg=bg, height=80)
        self.date_frame.pack(fill=tk.X, pady=8)
        self.date_frame.pack_propagate(False)
        self.date_label = tk.Label(
            self.date_frame, text="DATE: -",
            font=("Arial", 12, "bold"), bg=bg, fg=fg, justify=tk.LEFT,
        )
        self.date_label.pack(anchor="w", padx=6, pady=6)

        self.classify_frame = tk.Frame(self.right_col, bg=bg, height=120)
        self.classify_frame.pack(fill=tk.X, pady=8)
        self.classify_frame.pack_propagate(False)
        self.classify_label = tk.Label(
            self.classify_frame, text="CLASSIFICATION: -",
    ...
    def open_catalog_site(self) -> None:
        ...
        if url is None:
            messagebox.showerror(
                "Catalog site", "No free port found between 8000 and 8010."
            )
            return
        webbrowser.open(url)
        self.classify_label.config(
            text=f"CLASSIFICATION: Catalog site opened: {url}"
        )


    def show_ports(self) -> None:
        menu  = tk.Menu(self.root, tearoff=0)
        ports = self.arduino.available_ports() or [f"COM{i}" for i in range(1, 9)]
        for port in ports:
            menu.add_command(label=port, command=lambda p=port: self.select_port(p))
        menu.post(
            self.port_btn.winfo_rootx(),
            self.port_btn.winfo_rooty() + self.port_btn.winfo_height(),
        )
    ...
    def open_manual_commands_window(self) -> None:
        ...
        ttk.Label(
            frame, text="Robot commands", font=("Arial", 12, "bold")
        ...
        def run_command(cmd: str) -> None:
            if self.send_serial_command(cmd):
                status_var.set(f"Command sent: {cmd}")
                self.classify_label.config(
                    text=f"CLASSIFICATION: Manual command -> {cmd}"
                )
        ...
        ttk.Label(frame, textvariable=status_var, wraplength=280).pack(
            anchor="w", pady=(10, 0)
        )
    ...
    def emergency_stop(self) -> None:
        ...
        self.volume.set(0)
        self.classify_label.config(
            text="CLASSIFICATION: STOP sent to Arduino motors."
        )
    ...
    def apply_theme(self) -> None:
        if self.dark_mode_enabled:
            bg, fg, btn, slider_bg = (
                self.dark_bg, self.dark_fg, self.dark_btn, self.dark_slider_bg
            )
        else:
            bg, fg, btn, slider_bg = (
                self.light_bg, self.light_fg, self.light_btn, self.light_slider_bg
            )

        self.root.configure(bg=bg)
        for frame in (
            self.main, self.left_col, self.right_col,
            self.vol_frame, self.theme_frame,
            self.date_frame, self.classify_frame,
        ):
            frame.configure(bg=bg)
        for label in (
            self.label_init, self.label_vol, self.vol_value,
            self.date_label, self.classify_label, self.location_label,
        ):
            label.configure(bg=bg, fg=fg)
        for button in (
            self.place_btn, self.scan_btn, self.calibrate_btn,
            self.db_btn, self.site_btn,
            self.auto_btn, self.manual_btn,
            self.light_btn_theme, self.dark_btn_theme,
        ):
            button.configure(bg=btn, fg=fg)
        self.port_btn.configure(
            bg="#808080" if self.dark_mode_enabled else "#d9d9d9", fg=fg
        )
        self.location_entry.configure(bg=btn, fg=fg, insertbackground=fg)
        self.vol_slider.configure(
            bg=bg, fg=fg, troughcolor=slider_bg, highlightbackground=bg
        )
        self.stop_btn.configure(bg="#ff4444", fg="white")
        self.led_btn.configure(
            bg="#ffff00" if self.led_on else "#808080",
            fg="black"  if self.led_on else "white",
        )
    ...
    def update_camera_feed(self) -> None:
        ...
        if self.processing_capture:
            self.camera_after_id = self.root.after(
                CAMERA_REFRESH_MS, self.update_camera_feed
            )
            return
        try:
            ...
            if frame is not None:
                ...
                if self.camera.mm_per_pixel is None:
                    status_text = "NECALIBRAT"
                    color       = (0, 0, 255)
                else:
                    ref_px = self.camera.reference_diameter_px
                    status_text = (
                        f"CALIBRAT 23.75mm={ref_px:.1f}px"
                        if ref_px is not None
                        else f"CALIBRAT ({self.camera.mm_per_pixel:.4f} mm/pixel)"
                    )
                    color = (0, 255, 0)
                cv2.putText(
                ...
                cv2.putText(
                    display, "SPACE=capture | C=calibrate",
                    (10, display.shape[0] - 12),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1,
                )
                ok, ppm = cv2.imencode(".ppm", display)
                if ok:
            ...
            self.classify_label.config(
                text=f"CLASSIFICATION: Camera error: {exc}"
            )
            self.stop_camera()
            return
    ...
    def measure_diameter_from_live(
        self, sample_count: int = MEASUREMENT_FRAMES
    ) -> tuple[float | None, np.ndarray | None]:
    ...
    def confidence_diameter(
        self, d1: float, d2: float | None, tolerance: float = 1.2
    ) -> float:
    ...
    def format_match_list(matches: object) -> str:
        ...
        for match in matches:
            ...
            if not isinstance(row, dict):
                continue
            lines.append(
                f"- {row.get('denomination', '-')}: {float(confidence):.2f}%"
            )
        return "\n".join(lines)

    def _current_volume(self) -> float:
        return float(self.volume.get()) / 100.0

    def capture_and_catalog(self) -> None:
        if self.camera.current_frame is None:
            messagebox.showwarning(
                "Capture", "Start the camera (Place Artifact) before capturing."
            )
            return
        if self.processing_capture:
            return
    ...
    def _capture_worker(self) -> None:
        ...
        try:
            if self.camera.mm_per_pixel is None:
                msg    = "Press Calibrate first with the 23.75 mm coin in frame."
                result = {"status": "warning", "message": msg}
                self.tts.speak(msg, self._current_volume())
            else:
                diameter_mm, annotated = self.measure_diameter_from_live()
                if diameter_mm is None or annotated is None:
                    msg    = "Could not measure the coin reliably. Try again."
                    result = {"status": "warning", "message": msg}
                    self.tts.speak(msg, self._current_volume())
                else:
            ...
            result = {
                "status":  "error",
                "message": f"A measurement error occurred:\n{exc}",
            }
    ...
    def _capture_worker_done(self, result: dict[str, object]) -> None:
        try:
            ...
            if status == "warning":
                messagebox.showwarning(
                    "Capture", str(result.get("message", "Operation cancelled."))
                )
                return
            ...
            if best_conf > 80:
                denom    = (
                    best_row.get("denomination", "-")
                    if isinstance(best_row, dict)
                    else "-"
                )
                cls_text = f"Already exists ({best_conf:.2f}%) - {denom}"
                self.tts.speak(
                    f"Coin already catalogued: {denom}", self._current_volume()
                )
                ...
                if add_new:
                    ...
                    self.tts.speak(
                        "New coin added after review.", self._current_volume()
                    )
                else:
                ...
                self.tts.speak(
                    "New coin added automatically.", self._current_volume()
                )

            ts         = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
            pixel_line = ""
            if isinstance(scan_diameter_px, (float, int)) and isinstance(
                calibration_diameter_px, (float, int)
            ):
                pixel_line = (
                    f"\nCal px: {float(calibration_diameter_px):.1f}"
                    f" | Scan px: {float(scan_diameter_px):.1f}"
                )

            self.date_label.config(
                text=(
                    f"DATE: {ts}\n"
                    f"Diameter: {diameter_mm:.2f} mm{pixel_line}\n"
                    f"Image: {image_path.name}"
                )
            )
            self.classify_label.config(
                text=(
                    f"CLASSIFICATION: {cls_text}\nTop matches:\n{match_text}"
                    if match_text
                    else f"CLASSIFICATION: {cls_text}"
                )
            )
        finally:
            self.processing_capture = False
    ...
    def open_db_selected_image(self, tree: ttk.Treeview) -> None:
        ...
        if not selection:
            ...
            return
        image_ref = self.rows[tree.index(selection[0])].get("link", "").strip()
        if not image_ref:
            messagebox.showwarning("Database", "The selected row has no image.")
            return
        webbrowser.open(image_ref)

    def open_db_edit_dialog(
        self, parent: tk.Toplevel, tree: ttk.Treeview, mode: str
    ) -> None:
        selection      = tree.selection() if mode == "edit" else ()
        selected_id    = selection[0] if selection else None
        selected_index: int | None = tree.index(selected_id) if selected_id else None
        ...
        for idx, header in enumerate(self.headers):
            ttk.Label(form, text=header).grid(
                row=idx, column=0, sticky="w", padx=(0, 10), pady=5
            )
        ...
        def save_changes() -> None:
            ...
            if mode == "add":
                ...
                self.rows[selected_index] = new_row 
            self.save_database()
        ...
        btns = ttk.Frame(form)
        btns.grid(
            row=len(self.headers), column=0, columnspan=2,
            sticky="w", pady=(12, 0),
        )
        ttk.Button(btns, text="Save",   command=save_changes).pack(side="left")
    ...
    def delete_db_selected(self, tree: ttk.Treeview) -> None:
        ...
        if not messagebox.askyesno("Delete row", f"Delete '{denomination}'?"):
            return
        if not messagebox.askyesno(
            "Confirm delete",
            f"Are you sure you want to delete '{denomination}'?",
        ):
            return
        self.rows.pop(index)
        self.save_database()
    ...
    def open_database_window(self) -> None:
        ...
        for h in self.headers:
            ...
            tree.column(h, width=340 if h == "link" else 160, anchor="center")
        ...
        ttk.Button(
            actions, text="Open image",
            command=lambda: self.open_db_selected_image(tree),
        ).pack(side="left")
        ttk.Button(
            actions, text="Add",
            command=lambda: self.open_db_edit_dialog(win, tree, "add"),
        ).pack(side="left", padx=(8, 0))
        ttk.Button(
            actions, text="Update",
            command=lambda: self.open_db_edit_dialog(win, tree, "edit"),
        ).pack(side="left", padx=(8, 0))
        ttk.Button(
            actions, text="Delete",
            command=lambda: self.delete_db_selected(tree),
        ).pack(side="left", padx=(8, 0))
    ...
    def on_close(self) -> None:
        for attr in ("manual_window", "database_window"):
